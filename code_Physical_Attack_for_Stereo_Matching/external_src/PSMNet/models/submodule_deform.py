...
class DeformBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride, downsample, pad, dilation):
        ...
        self.conv2 = convbn_deform(planes, planes, 3, 1, pad, dilation)
...
class feature_extraction(nn.Module):
    def __init__(self, num_deform_layers=0):
        ...
        elif num_deform_layers == 6:
            self.layer1 = self._make_layer(BasicBlock, 32, 3, 1,1,1)
            self.layer2 = self._make_layer(BasicBlock, 64, 16, 2,1,1)
            self.layer3 = self._make_layer(DeformBlock, 128, 3, 1,1,1)
            self.layer4 = self._make_layer(DeformBlock, 128, 3, 1,1,2)
        elif num_deform_layers == 25:
            self.layer1 = self._make_layer(DeformBlock, 32, 3, 1,1,1)
            self.layer2 = self._make_layer(DeformBlock, 64, 16, 2,1,1)
            self.layer3 = self._make_layer(DeformBlock, 128, 3, 1,1,1)
            self.layer4 = self._make_layer(DeformBlock, 128, 3, 1,1,2)
        else:
            raise ValueError("[ERROR] Incorrect number of DeformBlocks")
        ...
        # Exposing end point for features, initialized as None
        self.end_point_features = None
    ...
    def forward(self, x):
        ...
        self.end_point_features = output
